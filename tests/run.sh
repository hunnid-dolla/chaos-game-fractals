...
if [ -z "$1" ]; then
    ...
    echo "${YELLOW}Использование: $0 <путь_к_директории> <путь_к_python_файлу>${NC}"
    exit 1
fi

# Проверяем, передан ли путь к python файлу
if [ -z "$2" ]; then
    echo "${RED}Ошибка: Не указан путь к python файлу.${NC}"
    echo "${YELLOW}Использование: $0 <путь_к_директории> <путь_к_python_файлу>${NC}"
    exit 1
fi
...
# Проверяем, существует ли python файл
if [ ! -f "$PYTHON_PATH" ]; then
    echo "${RED}Ошибка: python файл $PYTHON_PATH не найден.${NC}"
    exit 1
fi
